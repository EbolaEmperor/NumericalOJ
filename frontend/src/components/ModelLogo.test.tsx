...
afterEach(() => {
  cleanup()
  delete window.NumojModelFamily
})

describe('ModelLogo', () => {
  it('使用旧版模型厂商解析器生成并刷新 Logo', () => {
    const paint = vi.fn()
    window.NumojModelFamily = {
      iconClass: (name) => `model-family-logo model-family-logo--${name.startsWith('qwen') ? 'qwen' : 'deepseek'}`,
      paint,
    }
    ...
    expect(paint).toHaveBeenLastCalledWith(logo, 'qwen3.6-plus')
    ...
    expect(paint).toHaveBeenLastCalledWith(logo, 'deepseek-v4-pro')
  })
