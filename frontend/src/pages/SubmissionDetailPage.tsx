...
function WrittenResults({data, refresh}: {data: DetailResponse; refresh: () => void}) {
  ...
  return <>
    <section className="submission-detail-section"><header className="submission-detail-section-heading"><div><span>GRADING</span><h2>批改结果</h2></div>{isAdmin ? <div className="submission-detail-inline-actions">{Number(data.problem?.written_grading_mode || 1) === 4 ? <button type="button" className="submission-button submission-button--ghost" onClick={() => {setFaithsieveNotice(''); setFaithsieveOpen(true)}}><i className="fas fa-filter" aria-hidden="true" /> FaithSieve</button> : null}<button type="button" className="submission-button submission-button--ghost" id="openGradeEditorBtn" onClick={openEditor}><i className="fas fa-pen-to-square" aria-hidden="true" /> 编辑</button><button type="button" className="submission-button submission-button--ghost" id="nextBtn" disabled={next.isPending} onClick={() => next.mutate()}>下一个 <i className="fas fa-arrow-right" aria-hidden="true" /></button></div> : null}</header>{next.isError ? <div className="submission-action-feedback is-error" role="status">{next.error.message}</div> : faithsieveNotice ? <div className="submission-action-feedback is-success" role="status">{faithsieveNotice}</div> : null}<WrittenComment text={rawComment} /></section>
