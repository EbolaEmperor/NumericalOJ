...
function TestPointDetail({point, index, submissionId, lean, onOpenImage}: {point: JsonRecord; index: number; submissionId: number; lean: boolean; onOpenImage: (image: {url: string; alt: string}) => void}) {
...
function WrittenPdf({submission}: {submission: DetailResponse['submission']}) {
  ...
  return <div className="submission-pdf-surface" id="pdfViewer">{state === 'ready' ? <iframe title="submission-pdf" className="w-100 h-100 border-0" referrerPolicy="no-referrer" src={url} /> : <div className={`submission-pdf-hint ${state === 'error' ? 'text-danger' : 'text-muted'}`}>{state === 'error' ? '暂无可渲染的 PDF（可能是编译失败）。' : <MathCurveLoader size="lg" label={active ? 'PDF 生成中，请稍候...' : '正在加载 PDF…'} />}</div>}</div>
}
