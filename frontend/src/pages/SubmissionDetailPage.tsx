...
import {FaithSieveLaunchModal} from '../components/FaithSieveLaunchModal'
...
interface WrittenVote extends JsonRecord {index?: number; model?: string; status?: string; score?: number | null; comment?: string}
interface WrittenVoteAttempt extends JsonRecord {status?: string; needs_manual_review?: boolean; consensus_score?: number | null; manual_override?: boolean; manual_score?: number | null; votes?: WrittenVote[]}
interface DetailResponse extends ApiEnvelope {user?: {is_admin?: number}; submission: {id: number; problem_id: number; problem_title?: string; problem_type?: number; username?: string; status?: string; score?: number | null; code?: string; prompt_text?: string; generated_from_prompt?: boolean; prompt_generation_error?: string; created_at?: string}; problem?: ProblemSummary & {written_grading_mode?: number}; plang?: string; test_points: JsonRecord[]; lean_workspace?: LeanWorkspace | null; cached_ai_code_marks?: JsonRecord; submission_latex_text?: string; submission_latex_error?: string; submission_latex_html?: string; written_submission?: {show_latex_transcription?: boolean; vote?: WrittenVoteAttempt | null}}
...
export function mergeSubmissionDetailSnapshot(current: DetailResponse | undefined, snapshot: SubmissionStatusSnapshot) {
  ...
  return {
    ...
    written_submission: snapshot.written_vote === undefined ? current.written_submission : {...current.written_submission, vote: snapshot.written_vote as WrittenVoteAttempt | null},
  }
}

function detailSubmissionIsActive(data: DetailResponse | undefined) {
  if (!data || !submissionStatusIsActive(data.submission.status)) return false
  return !data.written_submission?.vote?.needs_manual_review
}
...
export function WrittenVoteResults({attempt, manualComment}: {attempt: WrittenVoteAttempt; manualComment: string}) {
  const slides = [
    ...(attempt.manual_override ? [{kind: 'manual', model: '人工复核', score: attempt.manual_score, comment: manualComment}] : []),
    ...(attempt.votes || []).map((vote) => ({kind: 'vote', model: String(vote.model || '未知模型'), score: vote.score, comment: String(vote.comment || ''), index: Number(vote.index || 0), status: String(vote.status || '')})),
  ]
  const [activeIndex, setActiveIndex] = useState(0)
  useEffect(() => {setActiveIndex(0)}, [attempt.id, attempt.manual_override])
  if (!slides.length) return null
  const active = slides[Math.min(activeIndex, slides.length - 1)]
  const activeSlideKey = 'index' in active ? `vote-${active.index}` : 'manual'
  ...
  return <div className="submission-vote-panel">{attempt.needs_manual_review && !attempt.manual_override ? <div className="submission-action-feedback is-warning" role="status">AI 评委给分不一致或有评委连续重试失败，当前结果不计入成绩，正在等待人工复核。</div> : null}{attempt.manual_override ? <div className="submission-action-feedback is-warning" role="status">人工复核已覆盖 AI vote 结果，默认展示人工评分。</div> : null}<div className="submission-vote-toolbar"><button type="button" className="submission-button submission-button--ghost" aria-label="上一位评委" disabled={slides.length < 2} onClick={previous}><i className="fas fa-chevron-left" /></button><div className="submission-vote-identity"><span>{active.kind === 'manual' ? 'MANUAL REVIEW' : `JUDGE ${'index' in active ? active.index : activeIndex + 1}`}</span><strong>{active.kind === 'vote' ? <ModelLogo model={active.model} /> : null}<span>{active.model}</span></strong></div><div className="submission-vote-score"><strong>{active.score ?? '—'}</strong><span>/ 5</span></div><button type="button" className="submission-button submission-button--ghost" aria-label="下一位评委" disabled={slides.length < 2} onClick={next}><i className="fas fa-chevron-right" /></button></div><div className="submission-vote-position">{activeIndex + 1} / {slides.length}</div><WrittenComment key={activeSlideKey} text={active.comment || (active.kind === 'vote' && 'status' in active && active.status === 'failed' ? '该评委连续重试后仍未完成。' : '')} /></div>
}
...
function WrittenResults({data, refresh}: {data: DetailResponse; refresh: () => void}) {
  ...
  const [faithsieveOpen, setFaithsieveOpen] = useState(false)
  const [faithsieveNotice, setFaithsieveNotice] = useState('')
  ...
  const voteAttempt = data.written_submission?.vote
  return <>
    <section className="submission-detail-section"><header className="submission-detail-section-heading"><div><span>GRADING</span><h2>批改结果</h2></div>{isAdmin ? <div className="submission-detail-inline-actions">{Number(data.problem?.written_grading_mode || 1) === 4 ? <button type="button" className="submission-button submission-button--ghost" onClick={() => {setFaithsieveNotice(''); setFaithsieveOpen(true)}}><i className="fas fa-filter" aria-hidden="true" /> FaithSieve</button> : null}<button type="button" className="submission-button submission-button--ghost" id="openGradeEditorBtn" onClick={openEditor}><i className="fas fa-pen-to-square" aria-hidden="true" /> 编辑</button><button type="button" className="submission-button submission-button--ghost" id="nextBtn" disabled={next.isPending} onClick={() => next.mutate()}>下一个 <i className="fas fa-arrow-right" aria-hidden="true" /></button></div> : null}</header>{next.isError ? <div className="submission-action-feedback is-error" role="status">{next.error.message}</div> : faithsieveNotice ? <div className="submission-action-feedback is-success" role="status">{faithsieveNotice}</div> : null}{voteAttempt ? <WrittenVoteResults attempt={voteAttempt} manualComment={rawComment} /> : <WrittenComment text={rawComment} />}</section>
    ...
    {isAdmin && Number(data.problem?.written_grading_mode || 1) === 4 ? <FaithSieveLaunchModal open={faithsieveOpen} action={`/api/admin/submissions/${submission.id}/faithsieve`} onClose={() => setFaithsieveOpen(false)} onQueued={(payload) => setFaithsieveNotice(String(payload.message || 'FaithSieve 任务已入队'))} /> : null}
  </>
}
...
export default function SubmissionDetailPage() {
  ...
  useEffect(() => {
    ...
    if (!data) return
    const active = detailSubmissionIsActive(data)
  ...
  const stream = useSubmissionStatusStream({
    ...
    enabled: result.isSuccess && detailSubmissionIsActive(result.data),
  ...
  const voteWaitingForManual = Boolean(data.written_submission?.vote?.needs_manual_review && !data.written_submission?.vote?.manual_override)
  const displayScore = voteWaitingForManual ? null : submission.score
  const accepted = displayScore != null && String(displayScore) === String(maxScore)
  const displayStatus = voteWaitingForManual ? '等待人工复核' : submission.status || 'Unknown'
