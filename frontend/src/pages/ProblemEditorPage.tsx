...
function WrittenVoteEditor({rows, items, onChange}: {rows: WrittenVoteRow[]; items: JsonRecord[]; onChange: (rows: WrittenVoteRow[]) => void}) {
  ...
  return <section className="border rounded p-3 mb-3" aria-label="AI 评委配置"><div className="d-flex align-items-center justify-content-between gap-3 mb-2"><div><div className="form-label mb-1"><i className="fas fa-users me-2" />AI 评委 Vote</div><div className="form-text mt-0">所有评委给分完全一致才自动结算；总计最多 10 票，并发最多 3 票。</div></div><button type="button" className="btn btn-outline-secondary btn-sm" disabled={rows.length >= 10} onClick={() => onChange([...rows, {endpoint_id: '', count: 1}])}><i className="fas fa-plus me-1" /> 添加模型</button></div><div className="d-grid gap-2">{rows.map((row, index) => <div className="row g-2 align-items-end" key={index}><div className="col-md-8"><label className="form-label">评委模型 {index + 1}</label><Choice value={row.endpoint_id} options={options} icon="fa-microchip" onChange={(value) => onChange(rows.map((item, rowIndex) => rowIndex === index ? {...item, endpoint_id: value} : item))} /></div><div className="col-8 col-md-2"><label className="form-label" htmlFor={`writtenVoteCount${index}`}>Vote 次数</label><input id={`writtenVoteCount${index}`} type="number" className="form-control" min={1} max={5} value={row.count} onChange={(event) => onChange(rows.map((item, rowIndex) => rowIndex === index ? {...item, count: Math.max(1, Math.min(5, Number(event.target.value) || 1))} : item))} /></div><div className="col-4 col-md-2"><button type="button" className="btn btn-outline-danger w-100" disabled={rows.length === 1} onClick={() => onChange(rows.filter((_, rowIndex) => rowIndex !== index))}><i className="fas fa-trash" /> <span className="visually-hidden">删除评委模型 {index + 1}</span></button></div></div>)}</div><div className={`form-text mt-2${total > 10 ? ' text-danger' : ''}`}>当前共 {total} 票{total > 10 ? '，超过上限' : ''}</div></section>
}
  ...
  return <section className="numoj-problem-editor-page">
    ...
    <form id={editing ? 'editProblemForm' : 'addProblemForm'} onSubmit={submit}>
      {editing && type === '1' ? <div className="row mb-3">
        ...
        <div className="col-md-3"><label className="form-label"><i className="fas fa-sliders me-2" /> 批改模式</label><Choice value={writtenMode} options={writtenModeOptions} icon="fa-sliders" onChange={changeWrittenMode} /></div>
      ...
      </> : <>{!editing ? <><p>书面作业：学生将通过提交文件来完成作业，老师需要人工批改。</p><div className="row g-3 mb-3"><div className="col-md-4"><label className="form-label"><i className="fas fa-sliders me-2" /> 批改模式</label><Choice value={writtenMode} options={writtenModeOptions} icon="fa-sliders" onChange={changeWrittenMode} /></div>{['1'].includes(writtenMode) ? <div className="col-md-4"><EndpointChoice name="ocr_endpoint_id" label="OCR 端点" value={values.ocr_endpoint_id || ''} items={endpointCandidates.ocr_endpoint_id || []} onChange={(value) => set('ocr_endpoint_id', value)} /></div> : null}</div></> : <div className="row g-3 mb-3">{['1'].includes(writtenMode) ? <div className="col-md-4"><EndpointChoice name="ocr_endpoint_id" label="OCR 端点" value={values.ocr_endpoint_id || ''} items={endpointCandidates.ocr_endpoint_id || []} onChange={(value) => set('ocr_endpoint_id', value)} /></div> : null}</div>}{writtenMode !== '4' ? <WrittenVoteEditor rows={writtenVoteRows} items={writtenVoteCandidates} onChange={setWrittenVoteRows} /> : null}{writtenMode !== '4' ? <div className="mb-3"><label className="form-label"><i className="fas fa-list-check me-2" /> 评分标准</label><textarea className="form-control" rows={6} placeholder="例如：步骤完整性 2 分、关键结论正确性 2 分、表达规范性 1 分；若缺失边界条件，最多给 4 分。" value={values.written_grading_prompt || (!editing ? query.data.options.default_written_grading_prompt : '') || ''} onChange={(event) => set('written_grading_prompt', event.target.value)} /></div> : null}{!editing ? <div className="mb-3"><label className="form-label"><i className="fas fa-check-circle me-2" /> 提交次数限制</label><input type="number" className="form-control" min={1} value={values.submission_limit || '10'} onChange={(event) => set('submission_limit', event.target.value)} /></div> : null}</>}
