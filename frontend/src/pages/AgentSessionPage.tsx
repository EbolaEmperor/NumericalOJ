...
export default function AgentSessionPage() {
  ...
  const judgeSession = session.category === 'judge' || session.task_kind === 'judge'
  ...
  const hardBlocked = judgeSession || !canResume || legacy || isBlockedStatus(status) || accessBlocked
  ...
  const blockedMessage = judgeSession ? 'Judge 会话由评测流程自动推进，不允许人工发送消息或插话。' : accessBlockedMessage || (legacy ? '旧任务没有可恢复的 workspace，无法继续会话' : !canResume ? '只有会话发起者可以继续发送消息' : isBlockedStatus(status) ? '身份配置清理失败，请管理员处理后再继续' : result.data?.can_retry_now ? '本轮未建立可恢复的原生会话，可以重试上一条消息' : '本轮未建立可恢复的原生会话，请新建 Agent 会话')
  ...
  </main>{renameOpen && !judgeSession ? <RenameModal sessionId={sessionId} initialTitle={String(session.title || 'Agent 会话')} close={() => setRenameOpen(false)} renamed={renameTitle} /> : null}</>
}
