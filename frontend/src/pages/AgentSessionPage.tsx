...
function traceKind(message: JsonRecord) {
  return String(message.kind || message.type || message.role || 'assistant').trim().toLowerCase().replaceAll('-', '_')
}
...
function TraceWorkDetail({message}: {message: JsonRecord}) {
  ...
  const title = String(message.title || message.name || message.tool_name || (
    ['thinking', 'reasoning'].includes(kind) ? '思考过程'
      : kind === 'tool' || kind === 'tool_call' ? '工具调用'
        : result ? failed ? '工具执行失败' : '工具结果'
          : kind === 'subagent' ? '子 Agent' : '工作详情'
  ))
...
export default function AgentSessionPage() {
  ...
  const currentTaskId = String(state.task_id || session.current_task_id || '')
  return <main className={`agent-session${workspaceOpen ? ' workspace-open' : ''}`} data-agent-session data-running={running} data-status={status}>
