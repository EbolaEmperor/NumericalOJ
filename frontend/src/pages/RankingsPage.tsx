...
export default function RankingsPage() {
  ...
  const [operationNotice, setOperationNotice] = useState<{message: string; error?: boolean} | null>(null)
  ...
  const copy = useMutation({
    mutationFn: (competitionId: number) => apiFetch<ApiEnvelope & {competition_id?: number}>(`/api/ranking/competitions/${competitionId}/copy`, {method: 'POST'}),
    onSuccess: async (data) => {setOperationNotice({message: data.message || `副本 #${String(data.competition_id || '')} 已创建`}); await queryClient.invalidateQueries({queryKey: ['rankings']})},
    onError: (error: Error) => setOperationNotice({message: error.message || '复制打榜赛失败', error: true}),
  })
  ...
  return <>
    ...
    {operationNotice ? <div className={`alert ${operationNotice.error ? 'alert-danger' : 'alert-success'}`} role={operationNotice.error ? 'alert' : 'status'}>{operationNotice.message}<button type="button" className="btn-close float-end" aria-label="关闭提示" onClick={() => setOperationNotice(null)} /></div> : null}
    ...
    {session?.user?.is_admin ? <>
      <button type="button" className="ranking-create-fab" onClick={() => setCreateOpen(true)} aria-label="创建打榜赛" title="创建打榜赛"><i className="fas fa-plus" aria-hidden="true" /></button>
      <ReactModal open={createOpen} onClose={() => setCreateOpen(false)} id="newCompetitionModal" labelledBy="newCompetitionLabel" dialogClassName="modal-lg"><div className="modal-content"><form onSubmit={submitCreate}><div className="modal-header"><h5 className="modal-title" id="newCompetitionLabel"><i className="fas fa-trophy me-2" /> 创建打榜赛</h5><button type="button" className="btn-close" onClick={() => setCreateOpen(false)} aria-label="关闭" /></div><div className="modal-body"><div className="mb-3"><label className="form-label">比赛标题</label><input type="text" className="form-control" name="title" required maxLength={255} placeholder="例如：2026 春季数值优化打榜赛" /></div><div className="mb-3"><label className="form-label">摘要</label><textarea className="form-control" name="summary" rows={2} maxLength={500} placeholder="一句话介绍本场打榜赛（展示在列表卡片上，最多 500 字）" /></div><div className="mb-3"><label className="form-label">满分</label><input type="number" className="form-control" name="max_score" defaultValue="100" min="1" max="100000" /></div><div className="mb-3"><label className="form-label">赛事描述（支持 Markdown）</label><textarea className="form-control" name="description" rows={6} placeholder="简介、规则、数据说明……" /></div>{create.isError ? <div className="alert alert-danger">{create.error.message}</div> : null}</div><div className="modal-footer"><button type="button" className="btn btn-outline-secondary" onClick={() => setCreateOpen(false)}>取消</button><button type="submit" className="btn btn-primary" disabled={create.isPending}><i className="fas fa-check me-1" /> {create.isPending ? '正在创建…' : '创建'}</button></div></form></div></ReactModal>
    </> : null}
  </>
}
