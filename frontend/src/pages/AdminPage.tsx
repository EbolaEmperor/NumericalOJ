...
function GradeRow({userId, grade}: {userId: number; grade: JsonRecord}) {
  ...
  return <tr><td><span className="user-admin-grade-copy"><strong>{String(grade.problem_title || '未命名题目')}</strong><small>P{String(grade.problem_id).padStart(4, '0')}</small></span></td><td><input className="user-admin-score-field" type="text" inputMode="numeric" value={score} aria-label={`${String(grade.problem_title || '未命名题目')}当前成绩`} onChange={(event) => setScore(event.target.value)} title={validationError || (save.isError ? errorMessage(save.error) : undefined)} /></td><td>{String(grade.max_score ?? '')}</td><td><button type="button" className="user-admin-grade-save" disabled={save.isPending} onClick={submit}>{save.isPending ? '保存中' : '保存'}</button></td></tr>
}
...
function GradesDialog({user, close}: {user: JsonRecord; close: () => void}) {
  ...
  return <><div className="modal fade show d-block user-admin-modal user-admin-grades-modal" role="dialog" aria-modal="true"><div className="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl"><div className="modal-content"><span className="user-admin-modal-accent" /><header className="modal-header"><div><span className="user-admin-kicker">LEARNING / SCORE RECORDS</span><h2 className="modal-title">成绩记录 · {String(user.username || '')}</h2><p>直接编辑已产生的题目成绩；留空保存会清除该条成绩。</p></div><button type="button" className="user-admin-modal-close" onClick={close} aria-label="关闭">×</button></header><div className="modal-body"><label className="user-admin-grade-search"><i className="fas fa-search" /><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索题号或题目标题" /></label>{result.isPending ? <div className="user-admin-loading">正在读取成绩</div> : result.isError ? <div className="user-admin-inline-state is-error">{result.error.message}</div> : <div className="user-admin-grades-table-wrap"><table className="user-admin-grades-table"><thead><tr><th>题目</th><th>当前成绩</th><th>满分</th><th>更新</th></tr></thead><tbody>{grades.length ? grades.map((item) => <GradeRow userId={Number(user.id)} grade={item} key={String(item.problem_id)} />) : <tr><td colSpan={4}><div className="user-admin-membership-empty">暂无匹配的成绩记录</div></td></tr>}</tbody></table></div>}</div><footer className="modal-footer"><span>修改将立即写入成绩记录</span><button type="button" className="user-admin-button user-admin-button-dark" onClick={close}>完成</button></footer></div></div></div></>
}
