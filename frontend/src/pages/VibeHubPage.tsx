...
export default function VibeHubPage() {
  ...
  const approve = useMutation({
    mutationFn: () => apiFetch<ApiEnvelope>(`/api/vibehub/admin/reviews/${encodeURIComponent(String(approveProject?.slug || ''))}`, {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({decision: 'approve', note: '', expected_version: Number(approveProject?.submitted_version)})}),
    onSuccess: async () => {setApproveStatus('审核已通过，正在刷新…'); await refresh(); setApproveProject(null)},
