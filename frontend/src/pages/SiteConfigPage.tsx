...
function ServiceForm({kind, settings, refresh}: {kind: 'mail' | 'web-search'; settings?: JsonRecord; refresh: () => Promise<unknown>}) {
  ...
  const save = useMutation({
    ...
    onSuccess: async (data) => {setForm(formFromSettings(data.settings)); await refresh()},
  })
  const test = useMutation({mutationFn: () => apiFetch<ApiEnvelope>(`/api/admin/dynamic-config/${kind}/test`, {method: 'POST', body: JSON.stringify(form)})})
  const clear = useMutation({mutationFn: () => apiFetch<ApiEnvelope>(`/api/admin/dynamic-config/${kind}`, {method: 'DELETE'}), onSuccess: async () => {setForm(formFromSettings()); await refresh()}})
  ...
  return <form ref={formRef} className="site-config-service-card" autoComplete="off" onSubmit={(event: FormEvent) => {event.preventDefault(); if (valid()) save.mutate()}}><header><span className="site-config-service-icon"><i className={`fas ${isMail ? 'fa-envelope' : 'fa-globe'}`} /></span><div><p>{isMail ? 'SMTP · SSL' : 'MCP · HTTP'}</p><h3>{isMail ? '邮件服务' : '联网搜索'}</h3></div><span className={`site-config-service-state${configured ? ' is-configured' : ''}`}>{configured ? '已配置' : '未配置'}</span></header><div className="site-config-form-grid">{isMail ? <><label className="wide"><span>SMTP 服务器</span><input required value={form.smtp_server} onChange={(event) => set('smtp_server', event.target.value)} /></label><label><span>端口</span><input required type="number" min={1} max={65535} value={form.smtp_port} onChange={(event) => set('smtp_port', event.target.value)} /></label><label><span>用户名</span><input required value={form.smtp_username} onChange={(event) => set('smtp_username', event.target.value)} /></label><label className="wide"><span>密码</span><input type="password" value={form.smtp_password} onChange={(event) => set('smtp_password', event.target.value)} placeholder={settings?.password_configured ? '留空保持原密码' : ''} /></label></> : <><label className="wide"><span>服务地址</span><input required type="url" value={form.base_url} onChange={(event) => set('base_url', event.target.value)} /></label><label className="wide"><span>认证信息</span><input type="password" value={form.authorization} onChange={(event) => set('authorization', event.target.value)} placeholder={settings?.authorization_configured ? '留空保持原认证信息' : ''} /></label></>}</div>{save.isError || test.isError || clear.isError || test.isSuccess || save.isSuccess || clear.isSuccess ? <p className={`site-config-form-status${test.isSuccess || save.isSuccess || clear.isSuccess ? ' is-success' : ' is-error'}`}>{test.isSuccess ? '连接测试成功' : save.isSuccess ? '配置已保存' : clear.isSuccess ? '配置已清除' : errorMessage(save.error || test.error || clear.error)}</p> : null}<footer><button className="site-config-danger-ghost" type="button" disabled={!configured || busy} onClick={() => {if (window.confirm(`确定清除整组${isMail ? '邮件' : '联网搜索'}配置？`)) clear.mutate()}}>清除</button><span className="site-config-action-spacer" /><button className="site-config-secondary" type="button" disabled={busy} onClick={() => {if (valid()) test.mutate()}}><i className={`fas ${isMail ? 'fa-paper-plane' : 'fa-satellite-dish'}`} />{test.isPending ? '测试中…' : '测试'}</button><button className="site-config-primary" type="submit" disabled={busy}>{save.isPending ? '保存中…' : '保存'}</button></footer></form>
}
...
export default function SiteConfigPage() {
  ...
  const [tab, setTab] = useState<'endpoints' | 'features' | 'other'>('endpoints')
  ...
  const updateBinding = useMutation({mutationFn: ({key, endpointId}: {key: string; endpointId: string}) => apiFetch<ApiEnvelope>(`/api/admin/dynamic-config/feature-bindings/${encodeURIComponent(key)}`, {method: 'PUT', body: JSON.stringify({endpoint_id: endpointId ? Number(endpointId) : null})}), onSuccess: refresh})
  ...
  const saveAgentConcurrency = useMutation({mutationFn: () => apiFetch<AgentConcurrencyResponse>('/api/admin/dynamic-config/agent-concurrency', {method: 'PUT', body: JSON.stringify({limit: parsedConcurrency})}), onSuccess: () => queryClient.invalidateQueries({queryKey: ['admin', 'dynamic-config', 'agent-concurrency']})})
  ...
  return <section className={`site-config-v2${tab === 'features' && (featurePending || featureError) ? ' is-feature-unavailable' : ''}`}>
