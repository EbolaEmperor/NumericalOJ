...
export function normalizeDeploymentCommit(raw: unknown = import.meta.env.VITE_NUMOJ_COMMIT_SHA) {
  const commit = String(raw || '').trim().toLowerCase()
  ...
  >
    ...
    <div className="site-config-feature-copy site-config-service-copy">
      ...
      <div className="site-config-form-grid">
        {isMail ? <>
          <label><span>SMTP 服务器</span><input name="smtp_server" required autoComplete="off" value={form.smtp_server} onChange={(event) => set('smtp_server', event.target.value)} /></label>
          ...
          <label><span>用户名</span><input name="smtp_username" required autoComplete="username" value={form.smtp_username} onChange={(event) => set('smtp_username', event.target.value)} /></label>
          <label><span>密码</span><input name="smtp_password" type="password" autoComplete="new-password" value={form.smtp_password} onChange={(event) => set('smtp_password', event.target.value)} placeholder={settings?.password_configured ? '留空保持原密码' : ''} /></label>
        </> : <>
          <label className="wide"><span>服务地址</span><input name="base_url" required type="url" autoComplete="off" value={form.base_url} onChange={(event) => set('base_url', event.target.value)} /></label>
          <label className="wide"><span>认证信息</span><input name="authorization" type="password" autoComplete="new-password" value={form.authorization} onChange={(event) => set('authorization', event.target.value)} placeholder={settings?.authorization_configured ? '留空保持原认证信息' : ''} /></label>
        </>}
      </div>
...
export default function SiteConfigPage() {
  ...
  const deploymentCommit = normalizeDeploymentCommit()
  return <section className={`site-config-v2${tab === 'features' ? ' is-features' : ''}${tab === 'features' && (featurePending || featureError) ? ' is-feature-unavailable' : ''}`}>
    ...
    <div className="site-config-workspace"><main className="site-config-content-scroll"><div className="site-config-panel-stage">
      ...
      <aside className={`site-config-function-rail${railOpen ? ' is-open' : ''}`} aria-label="配置分类" aria-hidden={!railOpen && undefined}>
        <button className="site-config-rail-close" type="button" onClick={() => setRailOpen(false)} aria-label="关闭配置分类"><i className="fas fa-times" /></button>
        <nav className="site-config-rail-nav" role="tablist">
          <p className="site-config-rail-group-label">CONFIGURATION</p>
          <button className={`site-config-rail-button${tab === 'endpoints' ? ' active' : ''}`} type="button" role="tab" aria-selected={tab === 'endpoints'} onClick={() => switchTab('endpoints')}><i className="fas fa-project-diagram" /><span>LLM 端点</span><span className="site-config-rail-count">{endpointItems.length}</span></button>
          <button className={`site-config-rail-button${tab === 'features' ? ' active' : ''}`} type="button" role="tab" aria-selected={tab === 'features'} onClick={() => switchTab('features')}><i className="fas fa-toggle-on" /><span>功能配置</span><span className="site-config-rail-count">{bindingItems.length + 2}</span></button>
        </nav>
        <div className="site-config-rail-runtime">
          <p>RUNTIME</p>
          <span><i /> LIVE{deploymentCommit ? <code className="site-config-deploy-commit" title={`完整 commit：${deploymentCommit}`} aria-label={`当前部署 commit ${deploymentCommit.slice(0, 6)}`}>{deploymentCommit.slice(0, 6)}</code> : null}</span>
        </div>
      </aside>
    </div>
