...
function AgentJudgeSettings({data, reverse}: {data: Response; reverse: boolean}) {
  ...
  const serverMainFingerprint = JSON.stringify({endpoints: data.aj_endpoints || [], timeout: numberValue(data.competition.agent_judge_timeout_seconds, 1800), finalizeTimeout: numberValue(data.competition.reverse_judge_finalize_timeout_seconds, 180)})
  ...
  const mainFingerprint = JSON.stringify({endpoints, timeout, finalizeTimeout})
  ...
  const save = useMutation({
    mutationFn: () => apiFetch<ApiEnvelope & {endpoints?: JsonRecord[]}>(`/api/ranking/competitions/${data.competition.id}/agent-judge/endpoints`, {method: 'POST', body: JSON.stringify({timeout_seconds: timeout, reverse_judge_finalize_timeout_seconds: finalizeTimeout, endpoints})}),
    onSuccess: (payload) => {const savedEndpoints = payload.endpoints || endpoints; setEndpoints(savedEndpoints); mainBaselineRef.current = JSON.stringify({endpoints: savedEndpoints, timeout, finalizeTimeout}); void queryClient.invalidateQueries({queryKey: ['ranking', String(data.competition.id)]})},
  })
  ...
  const openNew = (kind: 'main' | 'gate') => {setEditorError(''); setEditor({kind, index: -1, value: {harness: 'claude_code', source_mode: 'custom', protocol: 'anthropic', concurrency_limit: 1, status: 'enabled', context_window_tokens: 1000000, max_output_tokens: 384000, thinking_compatibility: true, input_price_per_million: '0', cached_input_price_per_million: '0', output_price_per_million: '0'}})}
  const applyEditor = () => {
    if (!editor) return
    ...
    if (['input_price_per_million', 'cached_input_price_per_million', 'output_price_per_million'].some((field) => !Number.isFinite(Number(next[field] ?? 0)) || Number(next[field] ?? 0) < 0)) {setEditorError('端点价格必须是有效的非负数'); return}
    ...
    if (editor.index < 0 && String(next.source_mode || 'custom') === 'global') {
      ...
      if (candidate) next = {...next, base_url: candidate.base_url || '', model: candidate.model || '', protocol: candidate.protocol || 'openai', thinking_compatibility: Boolean(candidate.thinking_enabled), input_price_per_million: candidate.input_price_per_million ?? '0', cached_input_price_per_million: candidate.cached_input_price_per_million ?? '0', output_price_per_million: candidate.output_price_per_million ?? '0'}
    }
  ...
  return <>
    ...
    {editor ? <ReactModal open onClose={() => {setEditorError(''); setEditor(null)}} id="ajeEditModal" labelledBy="ajeEditModalLabel" className="aje-modal" dialogClassName="modal-lg modal-dialog-scrollable"><div className="modal-content">
          ...
          <div className="modal-body"><div className={`aje-modal-grid${editor.index < 0 ? ' is-add' : ''}`}>
            ...
            {([['input_price_per_million', '输入价格'], ['cached_input_price_per_million', '缓存输入价格'], ['output_price_per_million', '输出价格']] as const).map(([field, label]) => <label className="aje-modal-field aje-col-4" key={field}><span>{label}（元 / 百万 Token）</span><input type="number" min={0} max="999999999999.99999999" step="0.00000001" required inputMode="decimal" aria-label={label} value={String(editor.value[field] ?? '0')} onChange={(event) => updateEditorValue({[field]: event.target.value})} /></label>)}
