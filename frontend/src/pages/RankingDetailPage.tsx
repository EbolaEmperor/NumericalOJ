  ...
  return <article className={`aj-sub ${statusClasses[status] || 's-muted'}`} data-submission-id={row.id} data-username={username.toLowerCase()}>
    ...
    <div className="aj-sub-r2">
      <div className="aj-sub-meta">
        ...
        {row.base_model ? <span className="aj-card-meta" title={`基座模型：${row.base_model}`}><span>{row.base_model}</span></span> : null}
...
function SubmitPanel({data, competitionId}: {data: Response; competitionId: string}) {
  ...
  return <section className="ranking-v2-tab ranking-v2-submit" id="rankingSubmitTab" data-ranking-tab-panel="submit">
    ...
    {isReverseJudge ? <div className="rj-submit-controls">
      <div className="rj-submit-switch"><label htmlFor="reverseSubmitModeTrigger"><i className="fas fa-upload me-2" />提交方式</label><input type="hidden" value={reverseMode} readOnly /><div className={`rk-choice rk-choice-sm${reverseModeOpen ? ' open' : ''}`}><button type="button" className="rk-choice-trigger" id="reverseSubmitModeTrigger" aria-haspopup="listbox" aria-expanded={reverseModeOpen} onClick={() => setReverseModeOpen((value) => !value)}><span className="rk-choice-trigger-main"><i className={`fas ${reverseMode === 'git' ? 'fa-code-branch' : 'fa-file-archive'}`} /><span>{reverseMode === 'git' ? 'Git 仓库' : 'ZIP 压缩包'}</span></span><i className="fas fa-chevron-down rk-choice-caret" /></button><div className="rk-choice-menu" role="listbox" aria-label="提交方式" hidden={!reverseModeOpen}>{[['git', 'Git 仓库', 'fa-code-branch'], ['zip', 'ZIP 压缩包', 'fa-file-archive']].map(([value, label, icon]) => <button type="button" className={`rk-choice-option${reverseMode === value ? ' active' : ''}`} role="option" aria-selected={reverseMode === value} key={value} onClick={() => {setReverseMode(value as 'git' | 'zip'); setReverseModeOpen(false)}}><span className="rk-choice-option-main"><i className={`fas ${icon}`} /><span className="rk-choice-option-name">{label}</span></span><i className="fas fa-check rk-choice-option-check" /></button>)}</div></div></div>
      <div className="rj-submit-switch rj-node-switch"><label htmlFor="reverseAgentEndpointTrigger"><i className="fas fa-robot me-2" />AI 节点</label><input type="hidden" value={endpointId} readOnly /><div className={`rk-choice rk-choice-sm${endpointOpen ? ' open' : ''}`}><button type="button" className="rk-choice-trigger" id="reverseAgentEndpointTrigger" aria-haspopup="listbox" aria-expanded={endpointOpen} aria-label={`AI 节点：${endpointLabel}`} onClick={() => setEndpointOpen((value) => !value)}><span className="rk-choice-trigger-main rj-endpoint-trigger-main">{selectedEndpoint ? <><i className={harnessIconClass(selectedEndpoint.harness)} aria-hidden="true" /><span className="rj-endpoint-harness-name">{endpointHarness}</span><span className="rj-endpoint-plus" aria-hidden="true">+</span><i className={`rj-endpoint-model-logo ${modelIconClass(endpointModel)}`} aria-hidden="true" /><span className="rj-endpoint-model-name">{endpointModel}</span></> : <><i className="fas fa-robot" aria-hidden="true" /><span className="rj-endpoint-empty-name">无可用节点</span></>}<span className="visually-hidden" aria-hidden="true">{endpointLabel}</span></span><i className="fas fa-chevron-down rk-choice-caret" /></button><div className="rk-choice-menu" role="listbox" aria-label="AI 节点" hidden={!endpointOpen}>{endpoints.map((item) => {const harness = endpointHarnessLabel(item.harness); const model = String(item.model || `节点 #${item.id}`); return <button type="button" className={`rk-choice-option${String(item.id) === endpointId ? ' active' : ''}`} role="option" aria-selected={String(item.id) === endpointId} key={String(item.id)} onClick={() => {setEndpointId(String(item.id)); setEndpointOpen(false)}}><span className="rk-choice-option-main rj-endpoint-option-main"><i className={harnessIconClass(item.harness)} aria-hidden="true" /><span className="rj-endpoint-harness-name">{harness}</span><span className="rj-endpoint-plus" aria-hidden="true">+</span><i className={modelIconClass(model)} aria-hidden="true" /><span className="rj-endpoint-model-name">{model}</span></span><i className="fas fa-check rk-choice-option-check" /></button>})}</div></div></div>
    </div> : null}
...
function modelIconClass(model: unknown) {
  const helper = typeof window === 'undefined' ? undefined : (window as Window & {NumojModelFamily?: {iconClass?: (name: string) => string}}).NumojModelFamily
  return helper?.iconClass?.(String(model || '')) || 'fas fa-microchip'
}
