import {ChoicePicker as RankingChoice} from '../components/ChoicePicker'
...
function SubmitPanel({data, competitionId}: {data: Response; competitionId: string}) {
  ...
  return <section className="ranking-v2-tab ranking-v2-submit" id="rankingSubmitTab" data-ranking-tab-panel="submit">
    ...
    {isReverseJudge ? <div className="rj-submit-controls">
      <div className="rj-submit-switch"><label htmlFor="reverseSubmitModeTrigger"><i className="fas fa-upload me-2" />提交方式</label><RankingChoice name="reverse_source" id="reverseSubmitModeTrigger" small value={reverseMode} onChange={(value) => setReverseMode(value as 'git' | 'zip')} label="提交方式" options={[{value: 'git', label: 'Git 仓库', icon: 'fa-code-branch'}, {value: 'zip', label: 'ZIP 压缩包', icon: 'fa-file-archive'}]} /></div>
...
function AgentJudgeSettings({data, reverse}: {data: Response; reverse: boolean}) {
  ...
  const save = useMutation({
    mutationFn: () => apiFetch<ApiEnvelope & {endpoints?: JsonRecord[]}>(`/api/ranking/competitions/${data.competition.id}/agent-judge/endpoints`, {method: 'POST', body: JSON.stringify({timeout_seconds: timeout, reverse_judge_finalize_timeout_seconds: finalizeTimeout, ...(!reverse ? {orchestration_mode: orchestration} : {}), endpoints})}),
