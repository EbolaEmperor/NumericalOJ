...
function progressTerminal(payload?: ProgressResponse) {
  return ['completed', 'done', 'error', 'failed', 'canceled', 'cancelled'].includes(String(payload?.progress?.stage || '').toLowerCase())
}
...
function ExportSubProgress({label, icon, progress, color}: {label: string; icon: string; progress: unknown; color: 'info' | 'success'}) {
  const item = progress && typeof progress === 'object' ? progress as JsonRecord : null
  if (!item) return null
  const percentage = Math.max(0, Math.min(100, Number(item.percentage || 0)))
  return <section className="mt-3" aria-label={label}><div className="d-flex justify-content-between mb-1"><strong><i className={`fas ${icon} me-2`} />{label}</strong><span>{percentage}%</span></div><div className="progress" style={{height: 16}} role="progressbar" aria-valuenow={percentage} aria-valuemin={0} aria-valuemax={100}><div className={`progress-bar bg-${color}`} style={{width: `${percentage}%`}}>{percentage}%</div></div><small className="text-muted">{String(item.message || '')}</small></section>
}
...
export default function HomeworkAdminPage() {
  ...
  const exportProgress = useQuery({
    ...
    retry: false,
    refetchInterval: (query) => exportTask && dialog === 'codes' && query.state.status !== 'error' && !progressTerminal(query.state.data) ? 500 : false,
  })
  ...
  const plagiarismProgress = useQuery({
    ...
    retry: false,
    refetchInterval: (query) => plagiarismTask && dialog === 'plagiarism' && query.state.status !== 'error' && !progressTerminal(query.state.data) ? 500 : false,
  })
  useEffect(() => {
    if (!progressTerminal(plagiarismProgress.data)) return
    void queryClient.invalidateQueries({queryKey: ['admin', 'homework', 'plagiarism-records', selectedClass]})
  }, [plagiarismProgress.data, queryClient, selectedClass])
  ...
  const exportTerminal = progressTerminal(exportProgress.data)
  const plagiarismTerminal = progressTerminal(plagiarismProgress.data)
  const autoDownloadedExport = useRef('')
  useEffect(() => {
    if (!exportTask || String(exportStage.stage) !== 'completed' || autoDownloadedExport.current === exportTask) return
    autoDownloadedExport.current = exportTask
    const link = document.createElement('a')
    link.href = `/api/admin/homework/exports/${encodeURIComponent(exportTask)}/download`
    link.download = ''
    document.body.appendChild(link)
    link.click()
    link.remove()
  }, [exportStage.stage, exportTask])

  if (!session?.user?.is_admin) return <ErrorState message="该页面仅管理员可访问" />
  if (page.isPending) return <LoadingState label="正在读取作业管理" />
  if (page.isError) return <ErrorState message={page.error.message} retry={() => void page.refetch()} />
    ...
    >
      ...
      <div className="table-responsive plagiarism-records-table-wrap">
        <table className="table table-sm align-middle plagiarism-records-table"><thead><tr><th className="plagiarism-select-col"><input className="form-check-input" type="checkbox" aria-label="全选抄袭记录" checked={Boolean(records.data?.records.length) && selectedRecords.length === records.data?.records.length} onChange={(event) => setSelectedRecords(event.target.checked ? (records.data?.records || []).map((item) => Number(item.id)) : [])} /></th><th>记录ID</th><th>用户</th><th>题目</th><th>提交ID</th><th><i className="fas fa-balance-scale me-1" />比较规则</th><th>相同用户名</th><th>标记时间</th></tr></thead><tbody>
          {records.isPending ? <tr><td colSpan={8}><div className="plagiarism-records-state"><MathCurveLoader size="sm" label="加载中…" /></div></td></tr> : records.isError ? <tr><td colSpan={8}><div className="plagiarism-records-state text-danger" role="alert"><i className="fas fa-times-circle" /><span>{errorMessage(records.error)}</span><button type="button" className="btn btn-sm btn-outline-danger" onClick={() => void records.refetch()}>重新加载</button></div></td></tr> : records.data.records.length ? records.data.records.map((item) => {
