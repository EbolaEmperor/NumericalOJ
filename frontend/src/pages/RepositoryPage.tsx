...
export default function RepositoryPage() {
  ...
  const backupConflict = async (mode: 'copy' | 'download') => {
    ...
    if (mode === 'copy') {
      try {
        await navigator.clipboard.writeText(saveConflict.snapshot.content)
      } catch {
        const textarea = document.createElement('textarea')
        textarea.value = saveConflict.snapshot.content; textarea.style.position = 'fixed'; textarea.style.opacity = '0'
