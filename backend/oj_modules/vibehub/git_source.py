...
def _run(arguments, *, root: Path, output: Path, limit: int = MAX_GIT_BYTES) -> None:
        ...
        try:
            while process.poll() is None:
                ...
                if quotas.logical_tree_bytes(root) > limit or os.fstat(stderr.fileno()).st_size > 1024**2:
                    raise GitSourceError("Git 仓库或归档超过 5 GiB 暂存上限")
                time.sleep(.1)
...
def git_package(url, ref=None, *, upload_root=None):
    ...
    try:
        ...
        _run(["--git-dir", str(repo), "rev-parse", "HEAD"], root=staging, output=output)
