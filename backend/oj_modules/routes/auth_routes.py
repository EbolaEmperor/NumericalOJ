...
from backend.oj_modules.security.credentials import hash_password, validate_username, verify_password
from backend.oj_modules.security.throttling import cooldown_active, rate_limit_hit
from backend.oj_modules.observability import (
    client_ip,
    emit_audit,
    request_audit_fields,
)
from backend.oj_modules.security.login_guard import safe_local_next
from backend.oj_modules.security.auth import invalidate_cached_browser_user
...
def login():
    ...
    if not isinstance(json_payload, dict):
        json_payload = {}
    next_candidate = (
        json_payload.get('next') or request.form.get('next')
    ...
    if request.method == 'POST':
        username = str(
            json_payload.get('username', request.form.get('username', ''))
            or ''
        ).strip()
        password = str(
            json_payload.get('password', request.form.get('password', ''))
            or ''
        ).strip()
