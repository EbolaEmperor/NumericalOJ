...
from backend.oj_modules.vibehub.maintenance import (
...
from backend.oj_modules.routes.admin_user_routes import admin_user_bp
from backend.oj_modules.routes.homework_routes import (
    homework_bp,
    init_homework_module,
)
from backend.oj_modules.routes.auth_routes import auth_bp, init_auth_module
from backend.oj_modules.routes.editor_language_routes import (
    editor_language_bp,
    init_editor_language_module,
)
from backend.oj_modules.routes.lean_routes import lean_bp, init_lean_module
from backend.oj_modules.routes.problem_core_routes import (
    init_problem_core_module,
    problem_core_bp,
)
from backend.oj_modules.problems.catalog import invalidate_problem_list_cache_for_class
from backend.oj_modules.classroom.dashboard import init_class_activity_cache
...
from backend.oj_modules.routes.agent_access_routes import (
    agent_access_bp,
    configure_agent_concurrency_runtime_applier,
)
from backend.oj_modules.security.login_guard import (
...
from backend.oj_modules.shared.static_delivery import (
    PrecompressedStaticFlask,
...
from backend.oj_modules.runtime.pending_recovery import (
...
def _load_secret_key():
    ...
    if key:
        return key
    key_path = str(PROJECT_ROOT / 'tmp' / 'secret_key')
    try:
...
def spa_asset(filename):
    ...
    return send_precompressed_directory(
        FRONTEND_DIST_ROOT / 'assets',
        filename,
...
def _spa_index_response():
    ...
    if not index_path.is_file():
        return Response(
            '前端尚未构建，请先在 frontend/ 运行 npm run build。',
            status=503,
            mimetype='text/plain',
        )
    response = send_from_directory(FRONTEND_DIST_ROOT, 'index.html')
