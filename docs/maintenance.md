...
源文件不存在、权限不足或历史归档无法安全解包时，跳过不可取得的材料并在会话及报告中明确记为缺失；不修改源权限、不删除源文件。无法枚举的历史目录另记入报告的 `discovery_missing`。目标写入失败、磁盘 I/O 错误、配额超限、数据库错误和核验不一致仍会中止部署。未完成会话续跑时，若源已不可读取，但上次成功注入的安全副本仍在，则保留该副本并重新计算摘要，标明原源不可复核；已完成会话只重新核验。批量注入仅用于排除其他写入者的 workspace，在批次前后扫描用量、逐文件按 inode 净增量检查配额并原子发布，避免每个文件都遍历整个目录。
