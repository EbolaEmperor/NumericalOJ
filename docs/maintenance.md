...
6. 只有回滚点验证成功后，才切换 `.deploy/current-venv`，执行一次非破坏性结构同步、`scripts/backfill_class_logos.py` 幂等补齐历史班级的缺失 logo 种子，再执行显式停机任务恢复，最后切换生产镜像标签。logo 回填只更新 `NULL`/空值，已有种子不得重生成；任一步骤失败都立即停止，不自动恢复数据库，也不自动重启业务服务。回滚使用本次部署已验证的数据库回滚点，或者在确认 DDL 已提交后向前修复。
