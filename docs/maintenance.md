...
- 默认运行路径所需包进入 `requirements.txt`，测试工具进入 `requirements-test.txt`，重量级非默认能力进入 `requirements-optional.txt`。
...
仓库以 `myojdb.sql` 为结构基线，`scripts/init_db_schema.py` 在启动前补齐缺失库/表/列/索引，并同步可识别的列类型。它使用 advisory lock，但**没有版本化 migration、迁移历史或自动 down migration**，也不表达删除、重命名、数据回填和复杂约束演进。
...
- Web 与 Celery 是两个发布边界；Python 变更按项目既定的 `web -> celery` 顺序重启，变更本身必须兼容短暂的混合版本窗口。
- Docker 变更单独构建并记录旧镜像 ID；长期应使用不可变版本标签，而不是只依赖 `latest`。
- schema 先采用向后兼容扩展，再发布读取新结构的代码；没有验证兼容窗口时不做不可逆 DDL。

### 发布步骤

1. 记录部署提交、当前进程、当前镜像 ID 和健康状态。
2. 数据变更先备份并验证恢复路径。
3. 安全增量同步代码，排除生产配置与运行数据。
4. 必要时构建镜像并完成镜像级 smoke。
5. 按明确 PID 停止应用的 web/celery supervisord，启动 `web.conf`，再启动 `celery.conf`。
6. 检查 `/health/live` 和 `/health/ready`，随后检查三个 worker、错误日志和一个只读业务路径。
7. 观察期内不清除旧代码包、旧镜像或备份。
...
- 持续验证当前 Gunicorn `gthread` 配置的流式响应、超时和优雅停机；在消除进程内状态前保持单 worker，扩大进程数前先做跨进程一致性改造。
