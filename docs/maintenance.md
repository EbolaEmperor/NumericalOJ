...
5. 按明确 PID 停止应用的 web/celery supervisord；运行结构同步，再以 `scripts/recover_pending_tasks.py --confirm-celery-stopped` 显式恢复未完成任务。
6. 启动 `web.conf`，再启动 `celery.conf`。
7. 检查 `/health/live` 和 `/health/ready`，随后检查三个 worker、错误日志和一个只读业务路径。
8. 观察期内不清除旧代码包、旧镜像或备份。
