...
补齐只面向已经导入的历史 Judge 会话。版本 2 整理全部已导入历史会话，同时修复最初占位正文，以及版本 1 写入的“历史提示词未保存”和合并输入；已保存的真实单轮正文保留，修正其最终回复。原生日志中保存的每次真实输入各建一个历史轮次，对应的回复、思考、工具工作详情随原事件 ID 分配到该轮；每轮最终回复取该轮实际最后一条 assistant。无法确定归属的事件单独放在“轮次未知”归档中，不猜测属于某次输入。事件正文保留，旧导入的聚合结论仍可在 workspace 的 `historical_record.json` 和补齐报告中追溯。

Agent Judge 未保存 user 事件时，按旧执行阶段、原生会话状态和冻结的统一前提示词恢复 setup、实际执行的规则或旧单轮输入；不为被跳过的规则生成消息。历史规则、结果文件名等从已迁入的框架材料恢复，缺少的动态变量在原位置标记；完全没有发送阶段依据时，明确展示“固定模板说明”，不声称已恢复真实轮次。反向 AI 作答的初次、超时收尾和恢复输入按实际记录分开。质量门禁依据归档结果区分 CLI 审核与更早的直接 API 请求；后者按同一次请求保留 system/user 角色，用审核材料恢复旧快照。审核标准与 `criteria_sha256` 不符时仅将标准原文标为缺失，不省略固定模板。历史模板保持当时的目录及措辞，新任务采用现行项目目录提示词。

每个会话的轮次、已发送消息、完成态运行快照、轨迹索引和版本标记在同一事务中更新，不派发任务、不重新评测、不记账或移动文件。原 task ID 和 current_task_id 保留在最后一轮，已有链接继续有效；原汇总 token 用量仍只保留一份，不伪造各轮消耗。原创建/更新时间保留。每个会话完成后记录 `historical_prompt_version=2`；同版本完成报告再次运行直接跳过，版本 1 的完成报告不会阻止版本 2 修复。

保留本次数据库备份和 `.deploy/judge-history-prompts.json` 报告。报告记录原正文、原聚合结论、生成的轮次 ID、各轮事件数量及无法恢复的信息。失败会回滚当前会话，重跑只处理未完成会话。若需回退，按本章生产恢复流程，从该部署前备份恢复报告涉及会话的 `agent_sessions`、`agent_session_turns`、`agent_session_messages`、`agent_task_runs`、`agent_trace_events` 与 `agent_trace_sync_state`，移除报告列出的新增历史轮次记录；不动费用表或 workspace，也不自动回灌整库。确认补齐完成后，在后续提交移除一次性脚本、两个冻结恢复辅助模块、专用测试及部署调用，保留报告。
