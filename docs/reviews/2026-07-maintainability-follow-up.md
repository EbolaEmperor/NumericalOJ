...
对照初始审阅，本轮治理后系统的可维护性约为 **6.8/10**，从“依赖个人记忆才能安全变更”进入了“已有明确边界、数据安全门禁、可重复部署流程和统一可观测性骨架，但核心数据模型和后端热点仍需持续治理”的阶段。

这不是主观调分：可验证的变化包括测试清库 fail-closed、原地一键部署与数据库备份、生产 DDL 单一所有者、依赖分层、统一 Redis 客户端、关键提交文件/数据库一致性、模板领域化、前端结构契约，以及版本化 JSON 日志、关联 ID、认证/提交审计和 daemon 日志采集。分数仍被 7 个超过 2000 行的后端文件、无版本账本的迁移器、可变 username 关系、应用组合根、遗留 `print`/宽泛捕获、完整 CSRF/CSP 和传递依赖锁明显限制。
...
| 部署与运维 | 3.5 | 7.0 | 已有原地一键部署、双槽运行环境、数据库备份、明确停启顺序和统一日志目录/采集器；自动代码回滚与外部日志留存仍缺失 |
...
| 自动化部署 | 已完成 | 运维人员在目标目录 `git pull` 后运行 `bash deploy.sh`，完成主机级互斥、遗留候选清理、候选 venv/镜像、原子数据库备份、新旧 Supervisor 安全停机、结构同步、任务恢复和稳定重启；脚本不耦合远端同步、环境白名单或测试逻辑 |
| 文档漂移 | 已完成 | README、开发约束、维护手册与实际路径一致；旧 DeepWiki 生成物停止入库 |
| 统一 Redis 客户端 | 已完成 | 文本、二进制、阻塞订阅三种 profile 统一超时，生产模块不再自行构造客户端 |
| 结构化日志与关联 ID | 本阶段完成 | `numericaloj.log` v1、request/task 上下文、认证/提交审计、单写轮转、运维 CLI 和 MySQL/Redis/Docker journald 采集已落地；遗留 `print()` 与宽泛捕获继续按故障面收口 |
...
| 复审发现 | 处理结果 |
| --- | --- |
| 远端 staging、固定环境白名单和工件激活偏离实际运维流程 | 改为在任意 checkout 目录原地执行；部署脚本不再包含 SSH、rsync、hostname、用户名、固定目录或 Git 状态判断 |
| 更新 Python 依赖可能污染仍在运行的进程 | 使用两个项目内 venv 槽轮换，只重建非活动槽，避免修改运行中解释器 |
| 镜像重建会提前改变运行中 worker 使用的 `latest` | 先构建带本次部署标签的两个候选镜像，停机并完成结构同步后才切换 `latest` |
| 首次部署到空数据库时备份语义不完整 | 备份明确记录“原数据库不存在”而非伪装成正常 dump |
...
4. **清理遗留日志债**：统一结构化骨架已经完成；后续把散落的 `print()` 和宽泛异常捕获按评测、AI、索引、恢复四个故障面迁入安全事件，并为关键错误补充分类与处置状态。
5. **数据库备份恢复演练**：`.deploy/backups/` 已具备容量预检、部署状态清单和持锁留存清理，只自动保留最近 2 个成功部署回滚点且不触碰失败现场；下一步应在独立 MySQL 实例定期执行恢复演练，并把结果与外部备份策略纳入告警。
