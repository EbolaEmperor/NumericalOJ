...
def get_problem_llm_endpoint_candidates():
    """读取一次全站端点列表并生成题目表单候选。

    延迟导入避免领域模块加载时访问数据库；HTTP 适配层也无需彼此反向导入。
    """

    from oj_modules.site_config.services import list_llm_endpoints

    return build_problem_llm_endpoint_candidates(
        list_llm_endpoints(include_secrets=False)
    )
