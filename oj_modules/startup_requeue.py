...
三个 supervisor 进程（web / celery / judger）重启后，Celery 队列里原本累积的任务
会丢失，导致数据库里仍处于“排队中 / 评测中”的提交永远卡住。本模块在 web 进程启动
时扫描 MySQL，把这些未完成的提交重新 .delay() 进队列。
...
重启三个 supervisor）。若只重启 web 进程而 Celery worker 仍在评测，理论上可能对某条
        ...
        try:
            ...
            if problem_type == 2:
                if written_task is not None:
                    written_task.delay(sub_id)
                    requeued += 1
            else:
                if evaluate_task is not None:
                    evaluate_task.delay(sub_id)
                    requeued += 1
        except Exception as e:
            ...
            if scoring_mode == 'elo':
                ...
                if elo_initial_burst_task is not None:
                    elo_initial_burst_task.apply_async(
                        args=[competition_id, sub_id], countdown=3,
                    )
                    requeued += 1
            else:
                if ranking_task is not None:
                    ranking_task.delay(sub_id)
                    requeued += 1
        except Exception as e:
