...
def safe_local_next(value):
    ...
    # 浏览器与代理可能在规范化 URL 时解码路径；同时检查有限层编码，避免
    # ``/%5cevil.example`` 一类值在下游被解释成协议相对地址。
...
def install_global_login_guard(app, *, user_loader):
    ...
    认证流程、静态资源与健康检查在读取会话用户前直接放行。其余请求只有
    ``user_loader`` 返回有效用户时才继续；API 始终返回 JSON 401，其他路径
    （包括尚未匹配到路由的路径）统一跳转到登录页。
    """
