...
1. 持有主机级锁并校验生产配置；若 `clangd --version` 或 `bwrap --version` 不可用，先模拟并通过 Debian APT 安装精确 candidate 版本，拒绝卸载、改动既有依赖或触碰宿主关键包；再清理异常中断遗留的受管候选镜像标签，在 `.deploy/venvs/` 的非活动槽安装固定生产依赖；
