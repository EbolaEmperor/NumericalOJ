...
| `run-ci.sh` | 在容器或 GitHub runner 上逐模块顺序跑 pytest（默认 unit -> db -> CLI e2e，也可传入模块路径），始终读取 tracked `config.py`，由调用方通过环境变量注入网络拓扑；逐模块打印结果并写 JUnit XML，任一模块失败则整体非零退出。 |
...
- DB/E2E 运行配置由 GitHub Actions 或测试 Compose 显式注入环境变量；不得读取、挂载或复制生产 `.env`，也不得改写 tracked `config.py`。
