...
def test_forum_markdown_is_sanitized_and_external_images_are_allowed():
...
def test_forum_markdown_resanitizes_formula_placeholders_after_restoring_them():
    rendered = render_forum_markdown(
        '[链接](<$x" onmouseover="alert(1)$>)'
    )
...
def test_forum_markdown_highlights_common_fenced_languages(language, source):
    rendered = render_forum_markdown(f"```{language}\n{source}\n```")

    assert f'class="codehilite language-{language}"' in rendered
    assert '<span class="' in rendered


def test_forum_markdown_keeps_dollar_syntax_inside_code_fences():
    rendered = render_forum_markdown(
        "正文 $x^2$。\n\n"
        '```bash\necho "$HOME $name"\n```\n\n'
        "```php\n<?php $foo = $bar; ?>\n```"
    ...
    assert "NUMOJFORUMMATH" not in rendered


def test_forum_markdown_marks_mermaid_as_safe_source_without_server_svg():
    rendered = render_forum_markdown(
        "```mermaid\n"
        "flowchart LR\n"
