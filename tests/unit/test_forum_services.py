...
from backend.oj_modules.forum import services as forum_services
from backend.oj_modules.api import forum_api
...
def test_forum_markdown_delegates_to_shared_renderer(monkeypatch):
    calls = []
    monkeypatch.setattr(
        forum_services,
        "render_rich_markdown",
        lambda content: calls.append(content) or "<p>共享结果</p>",
    )

    assert render_forum_markdown("讨论正文") == "<p>共享结果</p>"
    assert calls == ["讨论正文"]
