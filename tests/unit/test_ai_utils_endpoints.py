...
from oj_modules.llm_endpoints import LLMEndpointCategory, LLMEndpointSnapshot
...
def test_direct_endpoint_is_frozen_and_category_checked():
    snapshot = ai_utils.resolve_llm_endpoint_snapshot(
        endpoint_mapping(),
    ...
    with pytest.raises(RuntimeError, match="类别不兼容"):
        ai_utils.resolve_llm_endpoint_snapshot(
...
def test_endpoint_source_must_be_unambiguous():
    with pytest.raises(RuntimeError, match="必须且只能"):
        ai_utils.resolve_llm_endpoint_snapshot(purpose="测试")
    with pytest.raises(RuntimeError, match="必须且只能"):
        ai_utils.resolve_llm_endpoint_snapshot(
            endpoint_mapping(),
            endpoint_id=7,
...
def test_feature_binding_resolves_once(monkeypatch):
    from oj_modules import dynamic_config_services
...
def test_dangling_problem_endpoint_reports_id(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match=r"不存在或已删除（ID: 987）"):
        ai_utils.resolve_problem_llm_endpoint_snapshot(
            problem,
            "review_endpoint_id",
        )
...
def test_generic_text_and_vision_helpers_forward_snapshot(monkeypatch):
    ...
    monkeypatch.setattr(ai_utils, "call_text", fake_text)
    monkeypatch.setattr(ai_utils, "call_vision", fake_vision)

    assert ai_utils._call_llm_text(
        "prompt",
        text_endpoint,
        timeout=12,
        system_prompt="system",
    ) == "text result"
    assert ai_utils._call_llm_vision(
        "look",
...
def test_code_marks_resolves_global_text_endpoint_once(monkeypatch):
    ...
    monkeypatch.setattr(ai_utils, "resolve_llm_endpoint_snapshot", fake_resolve)
    monkeypatch.setattr(
        ai_utils,
        "_call_llm_text",
    ...
    result = ai_utils.generate_ai_code_marks_from_submission_context(
        problem_content="题目",
