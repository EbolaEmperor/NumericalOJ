(() => {
  ...
  const protocolLabels = { openai: 'OpenAI-compatible', anthropic: 'Anthropic-compatible' };
  const categoryLabels = { omni: '全模态', text: '纯文本', vision: '视觉理解', embedding: 'Embedding' };
  const thinkingLabels = { enable_thinking: 'enable_thinking', thinking_type: 'thinking.type', none: '不发送' };
  const featurePresentation = {
    ai_code_annotation: { icon: 'fa-highlighter', description: '分析代码并生成 AI 标注与讲解。' },
    code_image_analysis: { icon: 'fa-image', description: '理解程序运行产生的图片，辅助输出一致性分析。' },
    solution_agent: { icon: 'fa-terminal', description: '解题 Agent 的推理与代码生成端点。' },
    testdata_agent: { icon: 'fa-vials', description: '造数据 Agent 的分析与测试数据生成端点。' },
    agent_summary: { icon: 'fa-compress-alt', description: '长任务上下文与阶段结果的摘要端点。' },
    repository_query_summary: { icon: 'fa-search-plus', description: '代码仓库检索结果的摘要与问答端点。' },
    repository_structuring: { icon: 'fa-sitemap', description: '仓库内容结构化与实体抽取端点。' },
    repository_embedding: { icon: 'fa-vector-square', description: '仓库索引向量化端点；此绑定可单独加锁。' },
  };
  ...
  function toast(message, type = 'success') {
    ...
    item.innerHTML = `<i class="fas ${type === 'error' ? 'fa-exclamation-circle' : 'fa-check-circle'}"></i><span>${escapeHtml(message)}</span>`;
  ...
  function values(items, fallback) {
    ...
    return source.map((item) => typeof item === 'string' ? item : item.value || item.key).filter(Boolean);
  }
  ...
  function selectOptions(select, entries, labels, firstLabel = '') {
    const current = select.value;
    const list = values(entries, []);
    select.innerHTML = `${firstLabel ? `<option value="">${escapeHtml(firstLabel)}</option>` : ''}${list.map((value) => (
      `<option value="${escapeHtml(value)}">${escapeHtml(labels[value] || value)}</option>`
    )).join('')}`;
    if (list.includes(current) || (firstLabel && current === '')) select.value = current;
  }

  function initializeMetaControls() {
    selectOptions($('[data-endpoint-protocol-filter]'), state.meta.protocols, protocolLabels, '全部协议');
    selectOptions($('[data-endpoint-category-filter]'), state.meta.categories, categoryLabels, '全部类别');
    const form = $('[data-endpoint-form]');
    selectOptions(form.elements.protocol, state.meta.protocols, protocolLabels);
    selectOptions(form.elements.category, state.meta.categories, categoryLabels);
  }
  ...
  function filteredEndpoints() {
    const query = String($('[data-endpoint-search]').value || '').trim().toLowerCase();
    const protocol = $('[data-endpoint-protocol-filter]').value;
    const category = $('[data-endpoint-category-filter]').value;
    const lock = $('[data-endpoint-lock-filter]').value;
    return state.endpoints.filter((endpoint) => {
      const haystack = `${endpoint.name || ''} ${endpoint.model || ''}`.toLowerCase();
      if (query && !haystack.includes(query)) return false;
      if (protocol && endpoint.protocol !== protocol) return false;
      if (category && endpoint.category !== category) return false;
      if (lock === 'locked' && !endpoint.is_locked) return false;
      if (lock === 'unlocked' && endpoint.is_locked) return false;
      return true;
    });
  }
  ...
  function endpointCard(endpoint, position) {
    ...
    const apiKey = endpoint.api_key_configured ? 'API Key 已配置' : 'API Key 缺失';
    return `
      <article class="site-config-endpoint-card" data-endpoint-id="${Number(endpoint.id)}">
        <div class="site-config-endpoint-main">
          <div class="site-config-endpoint-top">
            <div>
              <span class="site-config-endpoint-number">NODE ${String(position + 1).padStart(2, '0')}</span>
              <h3 class="site-config-endpoint-name" title="${escapeHtml(endpoint.name)}">${escapeHtml(endpoint.name)}</h3>
            </div>
          ...
          <div class="site-config-endpoint-model"><small>MODEL</small><strong title="${escapeHtml(endpoint.model)}">${escapeHtml(endpoint.model)}</strong></div>
          <div class="site-config-endpoint-url"><small>BASE URL · ${escapeHtml(apiKey)}</small><span title="${escapeHtml(endpoint.base_url)}">${escapeHtml(endpoint.base_url)}</span></div>
        </div>
        <footer class="site-config-endpoint-foot">
          <span class="site-config-test-state ${status.className}" title="${escapeHtml(endpoint.test_message || '')}"><i></i>${escapeHtml(status.label)}</span>
          ${locked ? '' : `
            <button class="site-config-icon-button" type="button" data-endpoint-action="test" title="复测连接" aria-label="复测 ${escapeHtml(endpoint.name)}"><i class="fas fa-vial"></i></button>
            <button class="site-config-icon-button" type="button" data-endpoint-action="lock" title="加锁" aria-label="加锁 ${escapeHtml(endpoint.name)}"><i class="fas fa-lock"></i></button>
            <button class="site-config-icon-button" type="button" data-endpoint-action="edit" title="编辑" aria-label="编辑 ${escapeHtml(endpoint.name)}"><i class="fas fa-pen"></i></button>
            <button class="site-config-icon-button is-danger" type="button" data-endpoint-action="delete" title="删除" aria-label="删除 ${escapeHtml(endpoint.name)}"><i class="fas fa-trash-alt"></i></button>`}
        </footer>
        ${locked ? `<div class="site-config-lock-overlay"><button type="button" data-endpoint-action="unlock"><i class="fas fa-unlock-alt"></i> 解锁</button></div>` : ''}
      </article>`;
  }
  ...
  function renderEndpoints() {
    ...
    grid.innerHTML = list.map(endpointCard).join('');
  ...
  function featureCard(binding) {
    ...
    const presentation = featurePresentation[key] || { icon: 'fa-cog', description: '为此项全站能力选择模型端点。' };
    const selectedId = binding.endpoint_id == null ? '' : String(binding.endpoint_id);
    const candidates = candidatesFor(binding);
    const missing = selectedId && !state.endpoints.some((endpoint) => String(endpoint.id) === selectedId);
    const options = [
      '<option value="">未配置</option>',
      ...(missing ? [`<option value="${escapeHtml(selectedId)}" class="text-danger">端点已删除（ID: ${escapeHtml(selectedId)}）</option>`] : []),
      ...candidates.map((endpoint) => `<option value="${Number(endpoint.id)}">${escapeHtml(endpoint.name)} · ${escapeHtml(endpoint.model)}</option>`),
    ].join('');
    ...
    return `
      <article class="site-config-feature-card${lockable ? ' is-embedding' : ''}" data-feature-key="${escapeHtml(key)}">
        <span class="site-config-feature-icon"><i class="fas ${escapeHtml(presentation.icon)}" aria-hidden="true"></i></span>
        <div class="site-config-feature-copy">
          <h3>${escapeHtml(binding.label || meta.label || key)}</h3>
          <p>${escapeHtml(presentation.description)}</p>
          <div class="site-config-feature-control">
            <select data-feature-select aria-label="${escapeHtml(binding.label || meta.label || key)}端点" ${locked ? 'disabled' : ''}>${options}</select>
            <span class="site-config-feature-saving" data-feature-saving></span>
            ${lockable && !locked ? '<button class="site-config-icon-button site-config-feature-lock" type="button" data-feature-lock title="加锁"><i class="fas fa-lock"></i></button>' : ''}
          </div>
        </div>
        ${locked ? '<div class="site-config-lock-overlay"><button type="button" data-feature-unlock><i class="fas fa-unlock-alt"></i> 解锁</button></div>' : ''}
      </article>`;
  }
  ...
  function renderBindings() {
    ...
    state.bindings.forEach((binding) => {
      ...
      if (card) $('[data-feature-select]', card).value = binding.endpoint_id == null ? '' : String(binding.endpoint_id);
    });
  }
  ...
  function setThinking(enabled) {
    ...
    form.elements.thinking_enabled.value = enabled ? 'true' : 'false';
    button.setAttribute('aria-checked', enabled ? 'true' : 'false');
    $('[data-thinking-switch-label]', button).textContent = enabled ? '开启' : '关闭';
  }
  ...
  function updateThinkingControls(preserve = true) {
    ...
    const formatSelect = form.elements.thinking_format;
    const previous = preserve ? formatSelect.value : '';
    let formats = values(state.meta.thinking_formats, ['enable_thinking', 'thinking_type', 'none']);
    if (protocol === 'anthropic') formats = formats.filter((value) => ['thinking_type', 'none'].includes(value));
    if (category === 'embedding') formats = ['none'];
    formatSelect.innerHTML = formats.map((value) => `<option value="${escapeHtml(value)}">${escapeHtml(thinkingLabels[value] || value)}</option>`).join('');
    formatSelect.value = formats.includes(previous) ? previous : (formats.includes('none') ? 'none' : formats[0]);
    const hidden = category === 'embedding';
    $('[data-thinking-format-field]', form).hidden = hidden;
    const toggleHidden = hidden || formatSelect.value === 'none';
    $('[data-thinking-toggle-field]', form).hidden = toggleHidden;
    if (toggleHidden) setThinking(false);
    if (category === 'embedding' && protocol !== 'openai') form.elements.protocol.value = 'openai';
  }
  ...
  function openEndpointModal(endpoint = null) {
    ...
    form.elements.protocol.value = endpoint?.protocol || values(state.meta.protocols, ['openai'])[0];
    form.elements.category.value = endpoint?.category || values(state.meta.categories, ['text'])[0];
    form.elements.base_url.value = endpoint?.base_url || '';
    form.elements.api_key.value = '';
    form.elements.api_key.placeholder = endpoint?.api_key_configured ? '留空则保留当前 API Key' : 'API Key 必填';
    form.elements.model.value = endpoint?.model || '';
    updateThinkingControls(false);
    if (endpoint?.thinking_format && values(state.meta.thinking_formats, []).includes(endpoint.thinking_format)) {
      form.elements.thinking_format.value = endpoint.thinking_format;
    }
    updateThinkingControls(true);
    setThinking(Boolean(endpoint?.thinking_enabled) && form.elements.thinking_format.value !== 'none');
  ...
  async function testEndpoint(form, button) {
    ...
    if (!payload.endpoint_id && !payload.api_key.trim()) {
      form.elements.api_key.setCustomValidity('创建端点时必须填写 API Key');
    ...
    resultBox.textContent = '正在按端点类别执行真实最小请求…';
    try {
  ...
  async function saveEndpoint(form, button) {
    ...
    try {
      await request(id ? `/llm-endpoints/${id}` : '/llm-endpoints', { method: id ? 'PUT' : 'POST', body: payload });
  ...
  async function retestEndpoint(endpoint, button) {
    ...
    try {
      await request('/llm-endpoints/test', {
        method: 'POST',
        body: {
          endpoint_id: Number(endpoint.id), name: endpoint.name, protocol: endpoint.protocol,
          category: endpoint.category, base_url: endpoint.base_url, api_key: '', model: endpoint.model,
          thinking_enabled: Boolean(endpoint.thinking_enabled), thinking_format: endpoint.thinking_format,
        },
      });
      toast(`“${endpoint.name}”连接正常`);
    } catch (error) {
      toast(`“${endpoint.name}”复测失败：${error.message}`, 'error');
    } finally {
      setBusy(button, false);
  ...
  function openLockTarget(kind, item) {
    ...
    form.reset();
    $('.modal-title', form).textContent = kind === 'endpoint' ? `加锁 · ${item.name}` : '加锁 · Embedding 绑定';
  ...
  async function submitLock(form, button) {
    ...
    setBusy(button, true, '正在加锁…');
    try {
      await request(path, { method: 'POST', body: { reason } });
      modal('lockModal').hide();
      toast('配置已加锁');
      if (target.kind === 'endpoint') await loadEndpoints(); else await loadBindings();
    } catch (error) {
      toast(error.message, 'error');
    } finally {
      setBusy(button, false);
  ...
  function openUnlockTarget(kind, item) {
    ...
    const configuredPhrase = state.meta.unlock_confirmations?.[kind === 'endpoint' ? 'endpoint' : 'embedding_binding'];
    const phrase = configuredPhrase || REQUIRED_UNLOCK_PHRASE;
  ...
  async function submitUnlock(form, button) {
    ...
    const phrase = state.meta.unlock_confirmations?.[target.kind === 'endpoint' ? 'endpoint' : 'embedding_binding'] || REQUIRED_UNLOCK_PHRASE;
    ...
    try {
      await request(path, { method: 'POST', body: { password: form.elements.password.value, confirmation: form.elements.confirmation.value } });
  ...
  function openDelete(endpoint) {
    ...
    $('[data-delete-name]').textContent = endpoint.name;
  ...
  async function saveBinding(card, select) {
    ...
    try {
      const data = await request(`/feature-bindings/${encodeURIComponent(key)}`, {
        method: 'PUT', body: { endpoint_id: select.value ? Number(select.value) : null },
      });
      ...
      window.setTimeout(() => { if (indicator.isConnected) indicator.textContent = ''; }, 1800);
    } catch (error) {
  ...
  function settingsObject(payload) { return payload.settings === undefined ? payload : payload.settings; }
  ...
  function populateMail(payload) {
    ...
    form.elements.smtp_password.placeholder = settings?.smtp_password_configured ? '留空则保留当前密码' : '首次保存必须填写';
  ...
  function populateSearch(payload) {
    ...
    form.elements.authorization.placeholder = settings?.authorization_configured ? '留空则保留当前 Authorization' : '首次保存必须填写';
  ...
  async function saveService(kind, form, button) {
    ...
    try {
      ...
      if (kind === 'mail') populateMail(data); else populateSearch(data);
  ...
  async function testService(kind, form, button) {
    ...
    const body = formBody(form);
    if (kind === 'mail') body.smtp_port = Number(body.smtp_port);
    setBusy(button, true, kind === 'mail' ? '正在发送…' : '正在测试…');
    try {
  ...
  async function clearService(kind, button) {
    ...
    try {
      ...
      if (kind === 'mail') populateMail({ settings: null }); else populateSearch({ settings: null });
      toast(`${label}配置已清除`);
    } catch (error) {
      toast(error.message, 'error');
    } finally {
  ...
  async function loadMail() { populateMail(await request('/mail')); }
  async function loadSearch() { populateSearch(await request('/web-search')); }
  ...
  function openRail() {
    $('[data-config-rail]').classList.add('is-open');
    $('[data-config-rail-backdrop]').classList.add('is-open');
    $('[data-config-rail-toggle]').setAttribute('aria-expanded', 'true');
  }
  function closeRail() {
    $('[data-config-rail]').classList.remove('is-open');
    $('[data-config-rail-backdrop]').classList.remove('is-open');
    $('[data-config-rail-toggle]').setAttribute('aria-expanded', 'false');
  }

  const tabButtons = $$('[data-config-tab]');
  tabButtons.forEach((button, index) => {
    button.addEventListener('click', () => activateTab(button.dataset.configTab));
  ...
  $('[data-config-rail-close]').addEventListener('click', closeRail);
  $('[data-config-rail-backdrop]').addEventListener('click', closeRail);
  $$('[data-endpoint-search], [data-endpoint-protocol-filter], [data-endpoint-category-filter], [data-endpoint-lock-filter]')
    .forEach((control) => control.addEventListener(control.matches('input') ? 'input' : 'change', renderEndpoints));
  ...
  $('[data-mail-form]').addEventListener('submit', (event) => { event.preventDefault(); saveService('mail', event.currentTarget, event.submitter); });
  ...
  $('[data-search-form]').addEventListener('submit', (event) => { event.preventDefault(); saveService('web-search', event.currentTarget, event.submitter); });
  ...
  async function initialize() {
    ...
    try {
      ...
      $('[data-endpoint-loading]').innerHTML = `<i class="fas fa-exclamation-circle"></i> ${escapeHtml(error.message)}`;
      $('[data-feature-grid]').innerHTML = `<div class="site-config-empty"><i class="fas fa-exclamation-circle"></i><h3>配置读取失败</h3><p>${escapeHtml(error.message)}</p></div>`;
      toast(error.message, 'error');
    }
  }
