...
def _repository_version(client: Any, path: str, field: str) -> int:
    ...
    if not response_is_json(resp):
        raise CliError(f"Repository API response is missing JSON field {field}.")
    payload = resp.json()
    value = payload.get(field) if isinstance(payload, dict) else None
    if isinstance(value, bool) or not isinstance(value, int):
        raise CliError(f"Repository API response is missing integer field {field}.")
    return value
...
def necessary_repository_context_payload(payload: Any) -> Any:
    if not isinstance(payload, dict):
        return payload
    return {key: payload[key] for key in ("success", "allowed_extensions", "max_file_size_bytes", "defaults") if key in payload}
...
def necessary_repository_files_payload(payload: Any) -> Any:
    ...
    return {"success": payload.get("success", True), "files": files}


def necessary_repository_file_payload(payload: Any) -> Any:
    if not isinstance(payload, dict):
        return payload
    return {key: payload[key] for key in ("success", "id", "filename", "file_size", "updated_at", "content") if key in payload}


def necessary_repository_write_payload(payload: Any) -> Any:
    if not isinstance(payload, dict):
        return payload
    return {
        key: payload[key]
        for key in ("success", "message", "file_id", "filename", "file_size", "updated_at")
        if key in payload
    }


def necessary_repository_delete_payload(payload: Any) -> Any:
    if not isinstance(payload, dict):
        return payload
    return {
        key: payload[key]
        for key in ("success", "message", "file_id", "deleted")
        if key in payload
    }
...
def necessary_repository_index_status_payload(payload: Any) -> Any:
    if not isinstance(payload, dict):
        return payload
    if "job" in payload:
        return {"success": payload.get("success", True), "job": _necessary_repository_job(payload.get("job"))}
    return payload
...
def necessary_repository_active_status_payload(payload: Any) -> Any:
    ...
    return {
        ...
        "job": _necessary_repository_job(payload.get("job")) if payload.get("job") else None,
    }
...
def necessary_repository_search_payload(payload: Any) -> Any:
    ...
    return {"success": payload.get("success", True), "query": payload.get("query"), "hits": hits}
...
def necessary_repository_classes_payload(payload: Any) -> Any:
    ...
    return {"success": payload.get("success", True), "classes": classes, "count": len(classes)}


def repository_page(args: argparse.Namespace) -> None:
    client = client_from_args(args)
    resp = client.request("GET", "/api/repository/context")
    common.output_projected_json_response(resp, necessary_repository_context_payload)


def repository_files(args: argparse.Namespace) -> None:
    client = client_from_args(args)
    resp = client.request("GET", "/api/repository/files")
    common.output_projected_json_response(resp, necessary_repository_files_payload, allow_redirect=True)
...
def repository_get_file(args: argparse.Namespace) -> None:
    ...
    if args.output and response_is_json(resp) and resp.status_code < 400:
        ...
        return
    common.output_projected_json_response(resp, necessary_repository_file_payload, allow_redirect=True)
...
def repository_save_file(args: argparse.Namespace) -> None:
    ...
    common.output_projected_json_response(resp, necessary_repository_write_payload, allow_redirect=True)
...
def repository_delete_file(args: argparse.Namespace) -> None:
    ...
    if resp.status_code == 409 and response_is_json(resp):
        payload = resp.json()
    ...
    common.output_projected_json_response(resp, necessary_repository_delete_payload, allow_redirect=True)
...
def repository_build_index(args: argparse.Namespace) -> None:
    ...
    common.output_projected_json_response(resp, necessary_repository_index_start_payload, allow_redirect=True)
...
def repository_index_status(args: argparse.Namespace) -> None:
    ...
    common.output_projected_json_response(resp, necessary_repository_index_status_payload, allow_redirect=True)


def repository_active_status(args: argparse.Namespace) -> None:
    client = client_from_args(args)
    resp = client.request("GET", "/api/repository/index/status/active")
    common.output_projected_json_response(resp, necessary_repository_active_status_payload, allow_redirect=True)
...
def repository_search(args: argparse.Namespace) -> None:
    ...
    common.output_projected_json_response(resp, necessary_repository_search_payload, allow_redirect=True)
...
def repository_classes(args: argparse.Namespace) -> None:
    ...
    common.output_projected_json_response(resp, necessary_repository_classes_payload, allow_redirect=True)
...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    pa = add_cli_parser(repos, "page", "Fetch the repository page context as JSON.")
